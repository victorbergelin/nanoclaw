...
function createSchema(database: Database.Database): void {
  ...
  // Add summon_only column if it doesn't exist (migration for existing DBs)
  try {
    database.exec(
      `ALTER TABLE registered_groups ADD COLUMN summon_only INTEGER DEFAULT 0`,
    );
  } catch {
    /* column already exists */
  }
...
export function getRegisteredGroup(
  ...
  return {
    ...
    summonOnly: row.summon_only === 1 ? true : undefined,
  };
}
...
export function setRegisteredGroup(jid: string, group: RegisteredGroup): void {
  ...
  db.prepare(
    `INSERT OR REPLACE INTO registered_groups (jid, name, folder, trigger_pattern, added_at, container_config, requires_trigger, is_main, summon_only)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  ).run(
    jid,
    group.name,
